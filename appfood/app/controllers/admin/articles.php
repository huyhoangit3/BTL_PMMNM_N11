...
class Articles extends MY_Controller{
	...
	public function additem(){
		...
		if($this->input->post('add')){
			...
			$post_data = $this->lib_post->allows($post_data, array('title', 'slug', 'image', 'parentid', 'created', 'description', 'content', 'tags', 'publish', 'meta_title', 'meta_keyword', 'meta_description'));
	...
	public function edititem($id = 0){
		$id = (int)$id;
		$item = $this->db->select('title, slug, image, created, parentid, description, content, tags, publish, meta_title, meta_keyword, meta_description')->where(array('id' => $id))->from('articles_item')->get()->row_array();
	...
	public function editcategory($id = 0){
		$id = (int)$id;
		$category = $this->db->select('title, parentid, description, publish, level, meta_title, meta_keyword, meta_description')->where(array('id' => $id))->from('articles_category')->get()->row_array();
