...
class grouplink extends MY_Controller{
	...
	public function additem(){
		$data['meta_title'] = 'Thêm banner mới';
		$data['meta_keywords'] = '';
		$data['meta_description'] = '';
		if($this->input->post('add')){
			$post_data = $this->input->post('data');
			$post_data = $this->lib_post->allows($post_data, array('title', 'url', 'notes', 'image', 'parentid', 'publish'));
			...
			if($this->form_validation->run() == TRUE){
				...
				die($this->lib_common->js_redirect(CMS_BACKEND.'/grouplink/item'.CMS_SUFFIX, 'Thêm banner mới thành công.'));
			}
		}
	...
	public function edititem($id = 0){
		$id = (int)$id;
		$item = $this->db->select('title, url, notes, image, parentid, publish')->where(array('id' => $id))->from('grouplink_item')->get()->row_array();
		if(!isset($item) || count($item) == 0) die($this->lib_common->js_redirect(CMS_BACKEND.'/grouplink/item'.CMS_SUFFIX, 'banner không tồn tại.'));
		...
		if($this->input->post('add')){
			...
			if($this->form_validation->run() == TRUE){
				...
				die($this->lib_common->js_redirect(CMS_BACKEND.'/grouplink/item'.CMS_SUFFIX, 'Thay đổi thông tin banner thành công.'));
			}
		}
		else{
	...
	public function item(){
		$data['meta_title'] = 'Quản lý banner';
		$data['meta_keywords'] = '';
		$data['meta_description'] = '';
	...
	public function addcategory(){
		$data['meta_title'] = 'Thêm danh mục banner';
		$data['meta_keywords'] = '';
		$data['meta_description'] = '';
		...
		if($this->input->post('add')){
			...
			if($this->form_validation->run() == TRUE){
				...
				die($this->lib_common->js_redirect(CMS_BACKEND.'/grouplink/category'.CMS_SUFFIX, 'Thêm danh mục banner mới thành công.'));
			}
		}
		$data['data']['show_data']['parentid'] = $this->lib_nestedset->dropdown('grouplink_category', NULL, 'category');
		$data['menu_active'] = $this->menu_active;
	...
	public function editcategory($id = 0){
		...
		if(!isset($category) || count($category) == 0) die($this->lib_common->js_redirect(CMS_BACKEND.'/grouplink/category'.CMS_SUFFIX, 'Danh mục banner không tồn tại.'));
		...
		$data['meta_title'] = 'Thay đổi thông tin danh mục banner';
		$data['meta_keywords'] = '';
		$data['meta_description'] = '';
		if($this->input->post('add')){
			$post_data = $this->input->post('data');
			...
			if($this->form_validation->run() == TRUE){
				...
				$this->lib_nestedset->set('grouplink_category');
				die($this->lib_common->js_redirect(CMS_BACKEND.'/grouplink/category'.CMS_SUFFIX, 'Thay đổi thông tin danh mục banner thành công.'));
			}
		}
		else{
	...
	public function category(){
		$data['meta_title'] = 'Quản lý danh mục banner';
		$data['meta_keywords'] = '';
		$data['meta_description'] = '';
		$this->lib_nestedset->set('grouplink_category');
