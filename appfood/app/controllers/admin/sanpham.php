...
class Sanpham extends MY_Controller{
	...
	public function additem(){
		...
		if($this->input->post('add')){
			...
			$post_data = $this->lib_post->allows($post_data, array('title','gallery','video','price', 'price_market', 'image', 'parentid', 'description', 'content', 'tags', 'publish', 'meta_title', 'meta_keyword', 'meta_description'));
			...
			$this->form_validation->set_rules('data[price]', 'Giá sản phẩm', 'trim|numberic');
	...
	public function edititem($id = 0){
		$id = (int)$id;
		$item = $this->db->select('title, image,video, parentid,gallery,price, price_market, description, content, tags, publish, meta_title, meta_keyword, meta_description')->where(array('id' => $id))->from('sanpham_item')->get()->row_array();
		...
		if($this->input->post('add')){
			$post_data = $this->input->post('data');
			$post_data = $this->lib_post->allows($post_data, array('title','video', 'image','price', 'price_market', 'gallery', 'parentid', 'description', 'content', 'tags', 'publish', 'meta_title', 'meta_keyword', 'meta_description'));
			$data['data']['post_data'] = $post_data;
			$this->form_validation->set_error_delimiters('<li>', '</li>');
			$this->form_validation->set_rules('data[title]', 'Tiêu đề', 'trim|required');
			$this->form_validation->set_rules('data[parentid]', 'Node cha', 'trim|required|is_natural_no_zero');
	...
	public function addcategory(){
		...
		if($this->input->post('add')){
			$post_data = $this->input->post('data');
			$post_data = $this->lib_post->allows($post_data, array('title', 'parentid','image', 'description', 'publish', 'meta_title', 'meta_keyword', 'meta_description'));
			$data['data']['post_data'] = $post_data;
			$this->form_validation->set_error_delimiters('<li>', '</li>');
			$this->form_validation->set_rules('data[title]', 'Tiêu đề', 'trim|required');
	...
	public function editcategory($id = 0){
		$id = (int)$id;
		$category = $this->db->select('title, parentid,image, description, publish, level, meta_title, meta_keyword, meta_description')->where(array('id' => $id))->from('sanpham_category')->get()->row_array();
